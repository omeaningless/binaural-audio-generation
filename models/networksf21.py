...
def create_conv_sig(input_channels, output_channels, kernel, paddings, batch_norm=True, Sigmoid=True, stride=1):
    model = [nn.Conv2d(input_channels, output_channels, kernel, stride = stride, padding = paddings)]
    if(batch_norm):
        model.append(nn.BatchNorm2d(output_channels))
    if(Sigmoid):
...
class AudioNet1(nn.Module):
    def __init__(self, ngf=64, input_nc=2, output_nc=2, norm_mode='syncbn'):
        ...
        self.audionet_upconvlayer1 = unet_upconv(1296, ngf * 8,
                                                 norm_layer=norm_layer)  # 1296 (audio-visual feature) = 784 (visual feature) + 512 (audio feature)
...
class Attention(nn.Module):
    def __init__(self, input_nc =512, output_nc=512, kernel_size=1, norm_mode='syncbn'):
        ...
        self.conv1x1_1 = create_conv(512, 512, kernel_size, 0)
        self.conv1xa_2 = create_conv_sig(512, 512, kernel_size, 0)
